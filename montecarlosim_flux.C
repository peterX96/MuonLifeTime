...
int main(int argc, char** argv){
	int Nevents = std::atoi(argv[1]); // number of events
	...
	TFile* out_file = new TFile(("Flux_"+std::to_string(Nevents)+"_nevents.root").c_str(), "RECREATE");
	out_file->cd();

	TRandom3 *rg = new TRandom3();

	for (int i = 0; i < Nevents; i++){
		gen_x = rg->Uniform()*slab_length - slab_length/2;
